    ...
    private func sandboxCard(folder: SandboxFolder) -> some View {
        VStack(spacing: PryTheme.Spacing.sm) {
            Image(systemName: folder.icon)
                .font(.system(size: PryTheme.Size.diffLabel))
                ...
                ForEach(Array(items.enumerated()), id: \.element.id) { index, item in
                    ...
                    if index < items.count - 1 {
                        Divider()
                            ...
                            .padding(.leading, PryTheme.Size.methodColumn)
                    }
                }
            }
        }
    ...
    private func fileRow(item: FileItem) -> some View {
        HStack(spacing: PryTheme.Spacing.md) {
            Image(systemName: item.icon)
                ...
                .background(item.iconColor.opacity(PryTheme.Opacity.tint))
...
struct FilePreviewView: View {
    ...
    private func metadataRow(label: String, value: String) -> some View {
        HStack(alignment: .top) {
            Text(label)
                ...
                .frame(width: PryTheme.Size.metadataLabel, alignment: .leading)
    ...
    private var metadataDivider: some View {
        Divider()
            .overlay(PryTheme.Colors.border)
            .padding(.leading, PryTheme.Spacing.lg)
    }
