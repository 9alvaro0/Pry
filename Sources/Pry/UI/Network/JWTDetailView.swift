...
struct JWTDetailView: View {
    ...
    private func expirationBadge(_ jwt: JWTDecoder.DecodedJWT) -> some View {
        ...
        .foregroundStyle(jwt.isExpired ? PryTheme.Colors.error : PryTheme.Colors.success)
        .padding(.horizontal, PryTheme.Spacing.sm)
        .padding(.vertical, PryTheme.Spacing.xs)
        .background(
            (jwt.isExpired ? PryTheme.Colors.error : PryTheme.Colors.success).opacity(PryTheme.Opacity.tint)
        )
        .clipShape(.capsule)
    }
