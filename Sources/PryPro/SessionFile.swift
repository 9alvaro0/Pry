...
/// Exports and imports .pry session files.
enum SessionFileManager {
    ...
    /// Creates a .pry file from the current store and returns the temporary file URL.
    static func export(store: PryStore) -> URL? {
    ...
    /// Loads a .pry file and returns a read-only store with the session data.
    static func importSession(from url: URL) -> (store: PryStore, metadata: SessionFile.DeviceInfo)? {
